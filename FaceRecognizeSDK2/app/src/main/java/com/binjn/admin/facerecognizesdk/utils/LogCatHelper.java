...
public class LogCatHelper {
    ...
    private LogCatHelper(Context mContext,String path) {  
        ...
        if(TextUtils.isEmpty(path)){
            dirPath = Environment.getExternalStorageDirectory().getAbsolutePath()
                        + File.separator+File.separator+mContext.getPackageName();
        }else{  
            dirPath = path;  
        }  
    ...
    public void start(){  
        if(logThread == null){
            LogUtils.i("tag","启动log日志保存");
            logThread = new Thread(new LogRunnable(appid, dirPath));
           LogUtils.i("tag","logThread.name=" + logThread.getName() + ",id=" + logThread.getId());
        }
        logThread.start();  
    }
    ...
    private static class LogRunnable implements Runnable{
        ...
        public LogRunnable(int pid,String dirPath) {  
            ...
            Log.i("tag","------system log print start--------");
        }  
        ...
        public void run() {  
            try {  
                ...
                String line;
                Log.i("tag","-------system log print end--------");
